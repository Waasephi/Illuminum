...
{
	public class VileBolt : ModProjectile
	{
		...
		public override void AI()
		{
			...
			{
				var dust = Dust.NewDustDirect(Projectile.Center, 0, 0, DustID.Poisoned, -Projectile.velocity.X / 2f, -Projectile.velocity.Y / 2f);
				dust.noGravity = true;
				dust.fadeIn = 1f;
				dust.scale = 0.5f;
		...
		{                                                           // sound that the projectile make when hitting the terrain
			{
				...
				for (int i = 0; i < 6; i++)
                {
					...
					dust = Main.dust[Dust.NewDust(position, 0, 0, DustID.Poisoned, 0f, 0f, 0, new Color(255, 255, 255), 1f)];
					dust.noGravity = true;
					dust.scale = 0.5f;
				}
